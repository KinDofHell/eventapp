...
const Card = ({event, hasOrderLink, hidePrice}: CardProps) => {
    ...
    return (
        <div
            className="group relative flex flex-col min-h-[380px] w-full max-w-[400px] overflow-hidden rounded-xl bg-white shadow-md transition-all hover:shadow-lg md:min-h-[438px]">
            <Link
            ...
            <span className="p-semibold-14 w-min rounded-full bg-green-100 px-4 py-1 text-green-60">
              {event.isFree ? "FREE" : `$${event.price}`}
                ...
                <div className="flex-between w-full absolute bottom-4">
                    <p className="p-medium-14 md:p-medium-16 text-grey-500">
                        {event.organizer.firstName} {event.organizer.lastName}
                    </p>
                    {hasOrderLink && (
                        <Link href={`/orders?eventId=${event._id}`} className="flex gap-2 mr-10">
